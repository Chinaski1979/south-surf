...
if( !class_exists( 'ReduxFramework_gt3_registration' ) ) {
    ...
    class ReduxFramework_gt3_registration extends ReduxFramework {
        ...
        public function render() {
            ...
            echo '<input type="text" id="' . esc_attr($this->field['id']) . '_purchase_code" name="surfy[' . esc_attr($this->field['id']) . '][puchase_code]" value="'.$option_value['puchase_code'].'" class="regular-text "'
                        .($gt3_registration_status == 'active' ? ' readonly="readonly"' : '')
                        . esc_attr($this->field['class']) 
                . '>';
            echo '<div class="gt3_register__buttons">';
                echo '<a href="javascript:void(0);" class="gt3_register__submit">'.__( 'Activate', 'surfycore' ).'</a>';
                echo '<a href="javascript:void(0);" class="gt3_register__deregister">'.__( 'Deactivate', 'surfycore' ).'</a>';
                echo '<a href="javascript:void(0);" class="gt3_activation_refresh" title="'.__( 'Refresh Activation', 'surfycore' ).'"><i class="fa fa-refresh" aria-hidden="true"></i></a>';
            echo "</div>";


            if ($gt3_registration_status != 'active') {
                echo '<div class="gt3_info_container">'.__( 'To unlock all theme features and get auto-updates, please activate the theme. Enter your purchase code and click "Activate". Don\'t know where the purchase code is? ', 'surfycore' ).'<a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank">'.__( 'Click here.', 'surfycore').'</a>'.'</div>';
            }
            ...
            if ($gt3_registration_status == 'active') {
                ...
                if (get_option( 'gt3_account_attached' ) == 'true') {
                    echo "<div class='gt3_support_container gt3_info_container'>";
                        echo '<div><strong>'.esc_html( 'The purchase code is linked to your GT3themes account', 'surfy' ).' </strong></div>';
                        echo "<a class='button button-primary' href='https://gt3accounts.com/app' style='color: #71c341;background: #ffffff;border-color: #ffffff;box-shadow: none;text-shadow: none;color: #d54e21;font-weight: 600;margin-top: 5px;'>". esc_html( "Go to Your Account", "surfy" ) ." </a>";
                    echo "</div>";
                }else{
                    echo '<div class="gt3_info_container gt3_info_container--no_icon gt3_account_submit_container">';
                        echo "<div class='gt3_register__popup_sepparator'><span>".__( 'Next Step', 'surfycore' )."</span></div>";
                        echo '<strong style="font-size: 1.4em;color: #121212;">'.__( 'It\'s time to protect your purchase code.', 'surfycore' ).'</strong>';
                        ...
                        echo "<p>".__( 'Enter your email address to bind the purchase code. We will create a customer account for you by using your email address.', 'surfycore' )."</p>";
                        echo "<div>".__( 'After that, you will be able to:', 'surfycore' )."</div>";
                        echo "<ul>
                            <li>".__( 'View where your purchase code is used.', 'surfycore' )."</li>
                            <li>".__( 'Manage all your purchase codes.', 'surfycore' )."</li>
                            <li>".__( 'Check for product and support status.', 'surfycore' )."</li>
                        </ul>";
                        $admin_email = empty($option_value['email_account']) ? get_bloginfo('admin_email') : $option_value['email_account'];
                        echo '<input type="email"  id="' . esc_attr($this->field['id']) . '_account_email"  name="surfy[' . esc_attr($this->field['id']) . '][email_account]" value="'.$admin_email.'" class="regular-text gt3_account '.esc_attr($this->field['class']) .'">';
                        $newsletter_value = '';
                        echo '<a href="javascript:void(0);" class="gt3_account_submit">'.__( 'Protect & Create Account', 'surfycore' ).'</a>';
                    echo "</div>";
                }
                ...
                if (function_exists('gt3_get_support_time_left')) {
                    ...
                    if (!empty($support_time_left['time_to_left'])) {
                        ...
                        echo '<div><a class="button button-primary" href="'. $theme_link .'" style="color: #71c341;background: #ffffff;border-color: #ffffff;box-shadow: none;text-shadow: none;color: #d54e21;font-weight: 600;margin-top: 5px;">'. esc_html( "Update Support Package", "surfy" ) .' <i class="fa fa-angle-right" aria-hidden="true"></i></a></div>';
                        echo "</div>";
                    }
                }
            }
            ...
            echo "<div class='gt3_register__popup'>";
                ...
                echo '<div class="gt3_info_container gt3_info_container--no_icon gt3_account_submit_success_container">';
                    echo '<strong style="font-size: 1.4em;color: #121212;">'.__( 'Hey!', 'surfycore' ).'</strong>';
                    echo "<p>".__( 'Your account has been successfully created and its details were sent your e-mail address ', 'surfycore' )."<strong class='gt3_account_emai_holder'></strong></p>";
                    echo "<p>".__( 'You can log into your account at', 'surfycore' ).' <a href="https://gt3accounts.com/app">gt3accounts.com</a> '.__( 'to manage your purchase code(s) and get dedicated support.', 'surfycore' )."</p>";
                    echo "<p>".__( "If you haven't received your account details, please contact us at", 'surfycore' ).' <a href="mailto:help@gt3themes.com">help@gt3themes.com</a> '.__( "and our support team will assist you.", 'surfycore' )."</p>";
                    echo "<p><strong>".__( "Thank you for choosing our product.", 'surfycore' )."</strong></p>";
                    echo "<div class='gt3_account_submit_success_button'>".__( "Close", 'surfycore' )."</div>";
                echo "</div>";
            echo "</div>";
