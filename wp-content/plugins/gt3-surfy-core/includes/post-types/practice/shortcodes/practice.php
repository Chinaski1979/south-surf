...
class GT3Practice{
	...
	public function shortcodesMap(){
        if (function_exists('vc_map')) {
            vc_map( array(
                "name" => esc_html__("Practice List", "gt3_surfy_core"),
                "base" => $this->shortcodeName,
                "class" => $this->shortcodeName,
                "category" => esc_html__('GT3 Modules', 'gt3_surfy_core'),
                "icon" => 'gt3_icon',
                "content_element" => true,
                "description" => esc_html__("Practice List","gt3_surfy_core"),
                "params" => array(
                    array(
                        'type' => 'loop',
                        'heading' => esc_html__('Practice Items', 'gt3_surfy_core'),
                        ...
                        'description' => esc_html__('Create WordPress loop, to populate content from your site.', 'gt3_surfy_core')
                    ),
                    array(
                        'type' => 'dropdown',
                        'heading' => esc_html__('Items Per Line', 'gt3_surfy_core'),
                        'param_name' => 'posts_per_line',
                        'admin_label' => true,
                        'value' => array(
                            esc_html__("1", "gt3_surfy_core") => '1',
                            esc_html__("2", "gt3_surfy_core") => '2',
                            esc_html__("3", "gt3_surfy_core") => '3',
                            esc_html__("4", "gt3_surfy_core") => '4',
                        ),
                    ),
                    array(
                        'type' => 'dropdown',
                        'heading' => esc_html__('Select style', 'gt3_surfy_core'),
                        'param_name' => 'view_style',
                        'admin_label' => true,
                        'save_always' => true,
                        'value' => array(
                            esc_html__('Standard Pagination', "gt3_surfy_core") => "standard",
                            esc_html__('Ajax load more button', "gt3_surfy_core") => "ajax",
                        ),
                    ),
                    array(
                        'type' => 'checkbox',
                        'heading' => esc_html__( 'Show Filter', 'gt3_surfy_core' ),
                        'param_name' => 'show_filter',
                        'value' => array( esc_html__( 'Yes', 'gt3_surfy_core' ) => 'yes' ),
                        'std' => 'not',
                        'save_always' => true,
                    ),
                    array(
                        'type' => 'checkbox',
                        'heading' => esc_html__( 'Show Pagination', 'gt3_surfy_core' ),
                    ...
                    array(
                        'type' => 'checkbox',
                        'heading' => esc_html__( 'Show Load More Button', 'gt3_surfy_core' ),
                    ...
                    array(
                        'type' => 'textfield',
                        'heading' => esc_html__('Items on load', 'gt3_surfy_core'),
                        'param_name' => 'items_load',
                        'value' => '4',
                        'save_always' => true,
                        'description' => esc_html__( 'Items load by load more button.', 'gt3_surfy_core' ),
                        'dependency' => array(
                    ...
                    array(
                        'type' => 'checkbox',
                        'heading' => esc_html__( 'Show Content', 'gt3_surfy_core' ),
                    ...
                    array(
                        'type' => 'textfield',
                        'heading' => esc_html__('Charts in content', 'gt3_surfy_core'),
                    ...
                    array( 
                        'type' => 'checkbox',
                        'heading' => esc_html__( 'Show Read More Button', 'gt3_surfy_core' ),
                    ...
                    array(
                        'type' => 'checkbox',
                        'heading' => esc_html__( 'Module on dark background', 'gt3_surfy_core' ),
                        'param_name' => 'module_on_dark',
                        'value' => array( esc_html__( 'Yes', 'gt3_surfy_core' ) => 'yes' ),
                        'std' => 'not',
                    ),
                )
    ...
    public function render($atts, $content = null){
        ...
        $module_class = '';
        if ($module_on_dark == 'yes') {
            $module_class .= ' module_on_dark';
        }

        $out = '';
        $out .= '<div class="'.esc_attr($this->shortcodeName).$module_class.'">';
            if ($view_style == "ajax") {
            ...
            if($query_results->have_posts()):   
                ...
                $out .= '<p>'. _e( 'Sorry, no posts matched your criteria.', 'gt3_surfy_core' ) .'</p>';
            endif;  
            $out .= '</div>';
        ...
        if (strlen($wp_get_attachment_url)) {
            switch ($parameters['posts_per_line']) {
                case "1":
                    ...
                    break;
                case "2":
                    $gt3_featured_image_url = aq_resize($wp_get_attachment_url, "1170", "890", true, true, true);
                    break;
                case "3":
                    $gt3_featured_image_url = aq_resize($wp_get_attachment_url, "780", "592", true, true, true);
                    break;
                case "4":
                    $gt3_featured_image_url = aq_resize($wp_get_attachment_url, "586", "446", true, true, true);
                    break;
                default:
                    $gt3_featured_image_url = aq_resize($wp_get_attachment_url, "1170", "890", true, true, true);
            }
    ...
    public function getCategoriesOut($parameters){
        ...
        $out = '<a href="#" data-filter=".gt3_practice_list__item" class="active">'.esc_html__("All", "gt3_surfy_core").'</a>';
        foreach ($cats as $cat) {
            $out .= '<a href="'.get_term_link($cat->term_id, 'practice-category').'" data-filter=".item_'.$cat->term_id.'">';
            $out .= $cat->name;
            $out .= '</a>';
        }
        return $out;
    ...
    public function loadMorePractice ($parameters) {
        ...
        $x = esc_attr($x);
        $compile = '';
        $compile .= '</div><div class="clear"></div><div class="text-center"><a href="' . esc_js("javascript:void(0)") . '" class="load_more_works shortcode_button btn_normal btn_type3">' . esc_html__("Load More", "gt3_surfy_core") . '<i class="fa fa-angle-double-right"></i></a></div></div>';
            ...
            <script>
                (function($) {
                    ...
                    jQuery(window).load(function () {
                        /*gt3_get_posts("gt3_get_practice", "practice", "' . $post_count . '", posts_already_showed, "'.$posts_per_line.'", "' . $items_load . '");*/
                    ...
                    jQuery(".load_more_works").on("click", function(){
                        ...
                        gt3_get_posts("get_portfolio_practice", "practice", "' . $post_count . '", posts_already_showed,  "'.$posts_per_line.'", "' . $items_load . '", is_pres,"'.$x.'");
...
function gt3_get_practice_item ($parameters, $item_class) {
    $out = '';
    $post_cats = wp_get_post_terms(get_the_id(), 'practice-category');
        $post_cats_str = '';
            for ($i=0; $i<count( $post_cats ); $i++) {
                $post_cat_term = $post_cats[$i];
                $post_cat_name = $post_cat_term->term_id;
                $post_cats_str .= ' item_'.$post_cat_name;
            }
        ...
        $practice_icon = get_post_meta($p_id, "icon_selection", true);
        if ($practice_icon['select']) {
            $icon_output = '';
            $icon_output .= isset($practice_icon["select"]) && !empty($practice_icon["input"]) ? '<a class="icon-link" href="'.$practice_icon["input"].'"">' : '<div class="icon-link">';
            $icon_output .= '<i class="'.esc_attr($practice_icon["select"]).'"></i>';
            $icon_output .= isset($practice_icon["select"]) && !empty($practice_icon["input"]) ? '</a>' : '</div>';
        }
        $out .= '<article class="gt3_practice_list__item '.esc_attr($item_class).'">';
            $out .= '<div class="gt3_practice_list__image-holder">';
                ...
                $out .= '</a>';
                $out .= isset($icon_output) ? $icon_output : '';
            $out .= '</div>';
            $out .= '<div class="gt3_practice_list__content">';
                ...
                $out .= (bool) $parameters["show_readmore"] ? '<a href="'.get_permalink().'" class="gt3_practice_list__link learn_more">' . esc_html__("Learn More", "gt3_surfy_core") . '<span></span></a>' : '';
            $out .= '</div>';
        $out .= '</article>';
    return $out;
}
