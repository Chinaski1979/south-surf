...
function gt3_get_product_name (){
    $product = 'Surfy – WordPress Theme for Surf School Lessons and Clubs';
    return $product;
}
...
function gt3_account_activation($code = '',$email = ''){
    ...
    if (!empty($email) && function_exists('gt3_string_coding')) {
        ...
        if ( $response_code != 200 || is_wp_error( $version_info ) ) {
            $errore_return = new WP_Error( 'registration-error-connection-error', esc_html__( 'Registration Connection error', 'surfy' ) );
            return json_encode($errore_return);
        }
    }else{
        $errore_return = new WP_Error( 'registration-error-invalid-email', esc_html__( 'Please provide a valid email address.', 'surfy' ) );
        return json_encode($errore_return);
    }
...
if (get_option( 'gt3_registration_status') != 'active' && class_exists( 'Gt3_surfy_core' ) && get_option( 'gt3_supported_account_registration_srart') != 'false') {
...
function gt3_support_notice(){
    if (get_option('gt3_supported_notice_srart') == 'true' && class_exists( 'Gt3_surfy_core' )) {
...
function gt3_registration_notice() {
  ...
  <div class="notice notice-error" style="padding: 5px 25px 15px 75px;position: relative;">
    ...
    <p><?php esc_html_e( 'Purchase Validation! Please activate your theme.', 'surfy' ); ?></p>
    <div><a class="button button-primary" href="admin.php?page=<?php echo is_child_theme() == true ? 'Surfy-Child' : 'Surfy' ?>&tab=<?php echo (class_exists('WooCommerce') ? '18' : '15'); ?>"><?php esc_html_e( 'Register Now', 'surfy' ); ?> <i class="fa fa-angle-right" aria-hidden="true"></i></a> <a target="_blank" class="button button-primary" href="http://themeforest.net/cart/add_items?item_ids=22388606&ref=gt3themes"><?php esc_html_e( 'Buy Theme', 'surfy' ); ?> <i class="fa fa-angle-right" aria-hidden="true"></i></a> <a style="vertical-align: bottom;margin-left: 10px;line-height: 1.6;" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'gt3-hide-notice', 'account_registration_srart' ), 'gt3_hide_notices_nonce', '_gt3_notice_nonce' ) ); ?>"><?php esc_html_e( 'Dismiss this notice', 'surfy' ); ?></a></div>
  </div>
  <?php
  }
}
...
function gt3_registration_notice_supported_until() {
    ...
    if (!empty($support_time_left['notice_srart']) && $support_time_left['notice_srart']) {
        ...
        if (!empty($gt3_tmeme_id)) {
            $theme_link = 'https://themeforest.net/checkout/from_item/'.(int)get_option( 'gt3_tmeme_id' ).'?license=regular&size=source&support=renew_6month';
        }else{
  ...
  ?>
    <div class="notice notice-error is-dismissible" style="background-color: #d54e21; color: #ffffff; border-radius: 4px; padding: 10px 25px 10px 75px;position: relative;border-left: none;">
        ...
        if (!empty($support_time_left['expired']) && $support_time_left['expired'] == true) {
            esc_html_e( 'Your support package for this theme expired', 'surfy' ); ?><?php echo " ( ".$support_time_left['time_to_left']." ".esc_html( 'ago', 'surfy' )." ).";
        }else{
            esc_html_e( 'Your support package for this theme is about to expire', 'surfy' ); ?><?php echo " ( ".$support_time_left['time_to_left']." ".esc_html( 'left', 'surfy' )." ).";
        }
        ?></p>
        <div style="margin-bottom: 10px;"><a class="button button-primary" target="_blank" href="<?php echo esc_url($theme_link); ?>" style="color: #ffffff;background: #ffffff;border-color: #ffffff;box-shadow: none;text-shadow: none;color: #d54e21;font-weight: 600;margin-top: 5px;"><?php esc_html_e( 'Update Support Package', 'surfy' ); ?> <i class="fa fa-angle-right" aria-hidden="true"></i></a> <a class="button button-primary" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'gt3-hide-notice', 'notice_srart' ), 'gt3_hide_notices_nonce', '_gt3_notice_nonce' ) ); ?>" style="color: #ffffff;background: #ffffff;border-color: #ffffff;box-shadow: none;text-shadow: none;color: #d54e21;font-weight: 600;margin-top: 5px;"><?php esc_html_e( 'Dismiss this notice', 'surfy' ); ?> <i class="fa fa-angle-right" aria-hidden="true"></i></a></div>
    </div>
  <?php
    }
}
...
function gt3_hide_hotice (){
    if ( isset( $_GET['gt3-hide-notice'] ) && isset( $_GET['_gt3_notice_nonce'] ) ) {
        if ( ! wp_verify_nonce( $_GET['_gt3_notice_nonce'], 'gt3_hide_notices_nonce' ) ) {
            wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'surfy' ) );
        }
...
function gt3_account_notice(){
  ...
  <div class="notice notice-warning" style="padding: 10px">
    <p><strong><?php esc_html_e( 'Create account and manage your theme purchase codes.', 'surfy' ) ?></strong></p>
    <p><strong><a href="admin.php?page=<?php echo is_child_theme() == true ? 'surfy-Child' : 'surfy' ?>&tab=<?php echo (class_exists('WooCommerce') ? '18' : '15'); ?>"><?php esc_html_e( 'Register Now', 'surfy' ); ?></a> |  <a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'gt3-hide-notice', 'account_notice_srart' ), 'gt3_hide_notices_nonce', '_gt3_notice_nonce' ) ); ?>"><?php esc_html_e( 'Dismiss this notice', 'surfy' ); ?></a></strong></p>
  </div>
  <?php
}
