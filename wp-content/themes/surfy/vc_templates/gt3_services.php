<?php
	...
	if ($custom_content_size != '') {
		...
		$custom_content_line = $custom_content_size + 12;
	...
	$compile .= '<div class="gt3_services_box to-'.esc_attr($rotate_direction).' ' . esc_attr($animation_class) . ' ' . esc_attr($item_el_class) . (empty($icon) && empty($text) ? ' gt3_services_box--no_back' : '') . '"'.(empty($icon) && empty($text) ? ' style="' . esc_attr($module_height_style) . '"' : '').'>
		...
		if (!empty($icon) || !empty($text)) {
			$compile .= '<div class="gt3_services_box_content services_box-back" ' . (strlen($box_style) ? ' style="' . esc_attr($box_style) . '"' : '') . '>' . $icon .'' . $text .'<div class="fake_space"></div></div>';
		}		
		$compile .= '<a class="gt3_services_box_link" href="'.esc_attr($url).'" '.$title_for_button.' '.$button_target.'>' . esc_html__("View More", "surfy") . '</a>';
	$compile .= '</div>';
