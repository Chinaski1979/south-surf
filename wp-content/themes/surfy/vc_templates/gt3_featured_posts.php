...
if ($use_carousel == 'yes') {
    ...
    wp_enqueue_script('gt3-slick', get_template_directory_uri() . '/js/slick.min.js', array(), false, false);
    ...
    <div class="vc_col-sm-12 gt3_module_featured_posts blog_alignment_<?php echo esc_attr($content_alignment); ?> <?php echo esc_attr($animation_class); ?> <?php echo esc_attr($css_class); ?> blog_<?php echo esc_attr($view_type); ?> items<?php echo esc_attr($items_per_line); ?> <?php echo esc_attr($carousel_parent); ?> class_<?php echo esc_attr($rand_class); ?>" <?php echo (strlen($blog_style) ? 'style="' . esc_attr($blog_style) . '"' : ''); ?>>
        ...
        <?php
            ...
            if (strlen($module_title) > 0) {
                $moduletitle = '<div class="module_title_content" ' . (strlen($blog_title_headings) ? 'style="' . esc_attr($blog_title_headings) . '"' : '') . '>' . $module_title . '</div>';
            }
            if ($use_carousel == 'yes' && $use_prev_next_carousel !== 'yes') {
                $carousel_arrows = '<div class="carousel_arrows"><a href="'. esc_js("javascript:void(0)") .'" class="left_slick_arrow"><span></span></a><a href="'. esc_js("javascript:void(0)") .'" class="right_slick_arrow"><span></span></a></div>';
            }

            $module_title_block = $moduletitle . $external_btn . $carousel_arrows;

             echo (strlen($module_title) ? '<div class="gt3_module_title">' . $module_title_block . '</div>' : '');
        ?>
        ...
        if ($gt3_wp_query_in_shortcodes->have_posts()) {
            ...
            while ($gt3_wp_query_in_shortcodes->have_posts()) {
                ...
                if (strlen($post_title) > 0) {
                    ...
                    $post_link_title = '<h4 class="blogpost_title" ' . (strlen($blog_style_headings) ? 'style="' . esc_attr($blog_style_headings) . '"' : '') . '>' . $pf_icon . '<a href="' . esc_url(get_permalink()) . '" ' . (strlen($blog_style_headings) ? 'style="' . esc_attr($blog_style_headings) . '"' : '') . '>' . esc_html(get_the_title()) . '<span></span></a></h4>';
                }
					...
					<div class="blog_post_preview format-' . $pf . ' ' . $has_post_thumb . '">
                        <div class="item_wrapper">
                            <div class="blog_content">';
                                if (strlen($featured_image[0]) > 0 && $view_type !== 'type1') {
                                    $compile .= '<div class="blog_post_media"><a href="' . esc_url(get_permalink()) . '"><img src="' . esc_url(aq_resize($featured_image[0], $width, $height, true, true, true)) . '" alt="' . esc_attr(get_the_title()) .'" /></a>';
                                    if ($view_type == 'type3') {
                                ...
                                if ($post_read_more_link == 'yes') {
                                    $compile .= '<div><a href="'. esc_url(get_permalink()) .'" class="learn_more">'. esc_html__('Learn More', 'surfy') .'<span></span></a></div>';
                                }
