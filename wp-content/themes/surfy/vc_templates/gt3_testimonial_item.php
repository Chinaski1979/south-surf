<?php
	...
	if ($tstm_position != '') {
		$tstm_position_wrap = ',<div class="testimonial_item_position">' . esc_html($tstm_position) . '</div>';
	} else {
		$tstm_position_wrap = '';
	}
        ...
        <div class="testimonials_item">
            <div class="testimonial_item_wrapper">
                <div class="testimonials_content">
                    ...
                    (!empty($featured_image_url) &&  $view_type != "type1" ? '<div class="testimonials_photo"><img '.esc_attr($round_imgs).' src="' . aq_resize($featured_image_url, $img_width*2, $img_height*2, true, true, true) . '" alt="'. esc_attr__('Testimonial icon', 'surfy') .'" style="width:' . esc_attr($img_width) . 'px;" /></div>' : '' ) . '
                    <div class="testimonials_title"' . $sign_styles . '>' . esc_html($tstm_author) . $tstm_position_wrap .'</div>
                    ' .
                    (!empty($featured_image_url) &&  $view_type === "type1" ? '<div class="testimonials_photo"><img '.esc_attr($round_imgs).' src="' . aq_resize($featured_image_url, $img_width*2, $img_height*2, true, true, true) . '" alt="'. esc_attr__('Testimonial icon', 'surfy') .'" style="width:' . esc_attr($img_width) . 'px;" /></div>' : '' ) . '
                </div>
            </div>
