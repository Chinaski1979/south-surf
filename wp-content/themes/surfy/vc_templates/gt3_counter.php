<?php
	...
	if ( ! empty( $styles_google_fonts_counter_title ) ) {
		$counter_value_font = '' . esc_attr( $styles_google_fonts_counter_title ) . ';';
	} else {
		$counter_value_font = '';
	}

	if ( ! empty( $styles_google_fonts_counter_value ) ) {
		$counter_title_font = '' . esc_attr( $styles_google_fonts_counter_value ) . ';';
	} else {
		$counter_title_font = '';
	}
	...
	if ($icon_type == 'image') {
		if (strlen($featured_image_url))
			...
			if ($img_width != '') {
				...
				$imageblock_content .= '<div class="icon_container icon_proportions_' . esc_attr($image_proportions) . '"><img src="' . esc_url(aq_resize($featured_image_url, $img_width*2, $img_height, true, true, true)) . '" alt="'. esc_attr__('Counter icon', 'surfy') .'" style="width:' . esc_attr($img_width) . 'px;" /></div>';
			} else {
				$imageblock_content .= '<div class="icon_container"><img src="' . esc_url($featured_image_url) . '" alt="'. esc_attr__('Counter icon', 'surfy') .'" /></div>';
			}
	...
	$compile .= '<div class="stat_count_wrapper">';
		if($counter_value !== '') {
			$compile .= '<div class="stat_count" data-suffix="' . esc_attr($counter_suffix) . '" data-prefix="' . esc_attr($counter_prefix) . '" data-value="' . esc_attr($counter_value) . '" style="' . $counter_value_color_style . $counter_value_font . $counter_value_css .'">' . esc_html($counter_prefix) . esc_html($counter_value) . esc_html($counter_suffix) . '</div>';
		}
		if($counter_title !== '') {
