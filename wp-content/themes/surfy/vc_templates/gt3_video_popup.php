...
wp_enqueue_script('gt3-swipebox', get_template_directory_uri() . '/js/swipebox/js/jquery.swipebox.min.js', array(), false, false);
wp_enqueue_style('gt3-swipebox-style', get_template_directory_uri() . '/js/swipebox/css/swipebox.min.css');
...
$video_title = !empty($video_title) ? '<span class="video-popup__title" '.$title_style.' >'.$video_title.'</span>' : '';



if ( empty($bg_image) ):
	if (!empty($video_title)) {
		$video_custom_class = 'has_title_no_bg';
	} else {
		$video_custom_class = '';
	}
	?>
	<div class="video-popup-wrapper <?php echo esc_attr($video_custom_class)?>">
		<a class="video-popup__link swipebox-video" href="<?php echo esc_url($video_link); ?>" style="color: <?php echo esc_attr($btn_color)?>;"></a>
		<?php if (!empty($video_title)) { ?>
			<div><a class="swipebox-video" href="<?php echo esc_url($video_link); ?>"><?php echo(($video_title)); ?></a></div>
		<?php } ?>
	</div>
	<?php 
else:
