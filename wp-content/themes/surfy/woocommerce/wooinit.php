...
function gt3_add_label_outofstock () {
    ...
    if (!($product->is_in_stock())) {
        echo '<div class="gt3-product-outofstock"><span class="gt3-product-outofstock__inner">'.esc_html__('Out Of Stock', 'surfy').'</span></div>';
    }
}
...
/* Hide Second Title on Single Product Page */
...
function gt3_page_template () {
    ...
    if (class_exists( 'RWMB_Loader' ) && gt3_get_queried_object_id() !== 0 && !(class_exists('WooCommerce') && is_product_category())) {
...
function gt3_size_guide() {
    ...
    if (!empty($size_guide_url)) {
        echo '<div class="gt3_block_size_popup"><a href="#" class="image_size_popup_button">'.esc_html__('Size Guide', 'surfy').'</a></div><!-- gt3_block_size_popup -->';
    }
}
		...
		<?php if ( $previous_product ) : ?>
            <li>
                <a href="<?php echo esc_url( $previous_product->get_permalink() ); ?>" rel="prev">
					...
					echo '</span>';
					echo '<span class="nav_text">'.esc_html__('PREV', 'surfy').'</span>';
		...
		<?php if ( $next_product ) : ?>
            <li>
                <a href="<?php echo esc_url( $next_product->get_permalink() ); ?>" rel="next">
					...
					echo '</span>';
					echo '<span class="nav_text">'.esc_html__('NEXT', 'surfy').'</span>';
...
function woo_add_custom_general_field() {
    ...
    woocommerce_wp_checkbox( array(
        'id'            => '_checkbox_hot',
        'label'         => esc_html__( 'Hot Product', 'surfy' ),
        'description'   => esc_html__( 'Check for Hot Product', 'surfy' )
    ) );
    woocommerce_wp_checkbox( array(
        'id'            => '_checkbox_new',
        'label'         => esc_html__( 'New Product', 'surfy' ),
        'description'   => esc_html__( 'Check for New Product', 'surfy' )
    ) );
    echo '</div>';
}
...
function gt3_hot_new_product(){
    ...
    if ( 'yes' == $is_hot ) {
        echo '<span class="onsale hot-product">'.esc_html__('Hot','surfy').'</span>';
    }

    $is_new = get_post_meta( $product->get_id(), '_checkbox_new', true );
    if ( 'yes' == $is_new ) {
        echo '<span class="onsale new-product">'.esc_html__('New','surfy').'</span>';
    }
}
...
function gt3_variable_choose_an_option_rename( $args ){
    ...
    if (is_object($attr)) {
        ...
        $fix = esc_html__('an option','surfy');
    }
    $args['show_option_none'] = esc_html__('Choose ','surfy' ).$fix;
...
function gt3_woocommerce_pagination_args($args){
    $args['prev_text'] = '<i class="theme_icon-arrows-left"></i>';
    $args['next_text'] = '<i class="theme_icon-arrows-right"></i>';
    return $args;
}
